...
%   e.g.{'red','blue'}%insensitive to upper/lower letter
%   output is m x 3 matrix
...
function rgb = rgb(s)
  ...
  if strcmpi(s,'chart')
    showcolors()
      ...
      if ~iscell(s) s = {s}; end
      s = cellfun(@lower,s,'un',0);
    [flag,k] = ismember(s,name);%)find(strcmpi(s, name));
    if ~all(flag)
      error(['Unknown color: ' s{~(flag)}]);
    else
      rgb = num(k, :);
    end
  end
