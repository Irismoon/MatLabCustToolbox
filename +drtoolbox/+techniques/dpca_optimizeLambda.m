...
for rep = 1:options.numRep
    ...
    [Xtest, XtrainFull] = drtoolbox.techniques.dpca_getTestTrials(Xtrial, numOfTrials, ...
...
%     CnoiseTrain = diag(sum(bsxfun(@times, SSnoiseSumOverT(:,:), 1./(numOfTrials(:,:)-1)),2));
    ...
    if ~strcmp(options.noiseCovType, 'none')
        CnoiseTrain = drtoolbox.techniques.dpca_getNoiseCovariance(Xtrain, XtrainFull, numOfTrials-1, ...
    ...
    XtestMargs = drtoolbox.techniques.dpca_marginalize(XtestCen, 'combinedParams', options.combinedParams, ...
                    'ifFlat', 'yes');
    ...
    XtrainMargs = drtoolbox.techniques.dpca_marginalize(XtrainCen, 'combinedParams', options.combinedParams, ...
                    'ifFlat', 'yes');
    ...
    for l = 1:length(options.lambdas)
        ...
        [W,V,whichMarg] = drtoolbox.techniques.dpca(Xtrain, options.numComps, ...
